...
# ── GPU compatibility: ensure PyTorch supports the available GPU ──
# Must run BEFORE importing torch. Kaggle P100 (sm_60) is not supported
# by PyTorch 2.5+ (requires sm_70+). Detect and install compatible version.
import subprocess, sys, os

# Reduce CUDA memory fragmentation on P100
...
def _ensure_gpu_compat():
    try:
        ...
        if r.returncode == 0:
            ...
            if cc < 7.0:
                print(f"[GPU COMPAT] GPU compute capability {cc} < 7.0 — "
                      f"installing PyTorch 2.4.1+cu121 with P100 support...")
                subprocess.check_call([
                    sys.executable, "-m", "pip", "install", "-q",
                    "torch==2.4.1", "--index-url",
                    "https://download.pytorch.org/whl/cu121"
                ])
                print("[GPU COMPAT] PyTorch 2.4.1 installed. Continuing.")
            else:
                print(f"[GPU COMPAT] GPU compute capability {cc} — OK")
    except Exception as e:
        print(f"[GPU COMPAT] Check skipped: {e}")

_ensure_gpu_compat()
# ── End GPU compatibility ──

import math, json, time, random, logging
import numpy as np
import torch
...
OUTPUT_DIR = os.environ.get("PE_OUTPUT_DIR", "/kaggle/working")
...
if DEVICE == "cuda":
    ...
    log.info(f"GPU: {gpu_name}, compute capability: {cc[0]}.{cc[1]}")
    # P100 (cc 6.0) may not have kernel images in some PyTorch builds
    # Test a simple operation to verify CUDA works
    try:
        _test = torch.zeros(1, device="cuda") + 1
        del _test
    except RuntimeError as e:
        log.warning(f"CUDA test failed ({e}), falling back to CPU")
        DEVICE = "cpu"

@dataclass
class ModelConfig:
    vocab_size: int = 50257           # GPT-2 tokenizer
    n_layers: int = _env_int("PE_N_LAYERS", 12)                # 12 layers — deep enough for Jacobian expansion profiles
...
class TrainConfig:
    ...
    # Checkpoints for tracking gap emergence over training
...
def load_data_tokens(seq_len, max_tokens=100_000_000):
    """Load WikiText-103 and tokenize into a flat token array."""
    try:
...
def train(model, tokens_arr, config: ModelConfig, tcfg: TrainConfig, label: str,
    ...
    """Train model, optionally computing distances at checkpoints.
    
    Returns: (model, checkpoint_distances) where checkpoint_distances is a dict
    mapping step -> distance results (or empty dict if no checkpoints).
    """
    model.to(DEVICE)
    ...
    while step < tcfg.total_steps:
        ...
        # Compute distances at checkpoints (to track gap emergence)
...
def compute_distances(model, prompts, n_layers=12, n_prompts=300, max_gap=4):
    """Compute interchange and replacement distances for layer pairs with gap ≤ max_gap."""
    from transformers import GPT2TokenizerFast
    tokenizer = GPT2TokenizerFast.from_pretrained("gpt2")
    tokenizer.pad_token = tokenizer.eos_token
    ...
    results = {}
    pair_count = 0
    # Only compute pairs with gap ≤ max_gap (for 12 layers, all pairs = 66; gap≤4 = ~40)
...
def compute_jacobian_norms(model, prompts, n_layers=12, n_prompts=10, eps=1e-3):
    """Estimate per-layer Jacobian norms via finite-difference power iteration."""
    from transformers import GPT2TokenizerFast
    tokenizer = GPT2TokenizerFast.from_pretrained("gpt2")
    tokenizer.pad_token = tokenizer.eos_token
    ...
    for layer_idx in range(n_layers):
        # Get activations at layer input
        with torch.no_grad():
            ...
            # Power iteration for Jacobian norm estimate
...
def main():
    ...
    experiment_name = os.environ.get("PE_EXPERIMENT_NAME", "pe_ablation_v2")
    ...
    log.info("=" * 60)
    log.info(f"PE Ablation Experiment: {experiment_name}")
    log.info("=" * 60)
    ...
    log.info(
        ...
        tcfg.batch_size,
        tcfg.seq_len,
        list(tcfg.distance_checkpoints),
    )
    ...
    for model_type, use_rope in [("abspe", False), ("rope", True)]:
        ...
        # Train with checkpoint distance tracking
        ...
        # Free GPU memory before final distance computation (V22 OOM'd here with 300 prompts)
        torch.cuda.empty_cache() if DEVICE == "cuda" else None

        # Compute final distances (adjacent pairs, gap ≤ 4)
        log.info(f"Computing final pairwise distances for {model_type.upper()}...")
        distances = compute_distances(model, eval_prompts, n_layers=mcfg.n_layers, n_prompts=final_prompt_count, max_gap=4)

        # Compute Jacobian norms
        log.info(f"Computing Jacobian norms for {model_type.upper()}...")
        jacobian_norms = compute_jacobian_norms(model, eval_prompts, n_layers=mcfg.n_layers, n_prompts=jacobian_prompt_count)
        ...
        # Track gap emergence across checkpoints
    ...
    # Emergence trajectory comparison
    ...
    # Also check for trend: is RoPE ratio decreasing relative to AbsPE over training?
    gap_trending = False
